...
class ServiceReport():
    ...
    def __init__(self,name,trust_effective_date,report_period):
        ...
        self.wb_save_results = path_root  + '/../CheckTheseProjects/' + self.name + '/' + self.name + '.xlsx'
    ...
    def get_ServiceReportAssetsList(self,FilePath,AllAssetList,DefaultAssetList,WaivedAssetList,RedemptionAssetList):
    ...
    def check_OutstandingPrincipal(self):
        ...
        DetailList = self.service_report_AllAssetList
        DetailList = DetailList[['订单号','剩余本金','B1：正常回收','B2：提前还款','B3：拖欠回收','B4：违约回收','B5：账务处理']]
        DetailList['剩余本金_poolcutdate_calc'] = DetailList['剩余本金']+DetailList['B1：正常回收']+DetailList['B2：提前还款']+DetailList['B3：拖欠回收']+DetailList['B4：违约回收']+DetailList['B5：账务处理']
        DetailList['#合同号'] = '#' + DetailList['订单号'].astype(str)
        return DetailList
...
#        DetailList.to_csv(path_root  + '/../CheckTheseProjects/' +self.name+'/' + 'check_五级分类_Jonah.csv')
    ...
    def service_report_cal(self):
        ...
        cal_table_4_1(self.service_report_AllAssetList,self.wb_save_results)
