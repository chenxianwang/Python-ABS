        ...
        if self.RevolvingDeal is not True:
            ...
            for scenario_id in self.scenarios.keys():
                ...
                for which_revolving_pool in range(1,len(self.date_revolving_pools_cut) + 1):
                    ...
                    self.apcf_revolving[scenario_id][which_revolving_pool],df_ppmt,df_ipmt = cash_flow_collection(apcf_revolving_structure,dates_recycle_list_revolving,'first_due_period_R','Revolving'+str(which_revolving_pool),wb_name)
                ...
                #logger.info('sum([self.AP_PAcc_original[scenario_id][k] for k in dates_recycle])] is {0}'.format(sum([self.AP_PAcc_original[scenario_id][k] for k in dates_recycle])))
                    ...
                    self.amount_default_all[scenario_id] += deepcopy(self.AP_PAcc_loss_allTerm_R[scenario_id][which_revolving_pool][dates_recycle_list_revolving[-1]])
